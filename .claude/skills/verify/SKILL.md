---
name: verify
description: このリポジトリの変更を実測で確かめる。lint / build / generate と生成物の検査・dev での目視を、コマンドと出力を残す形で行う。「動くはず」で報告しないための手順。issue 対応や UI 変更の確認フェーズから呼ぶ。実測だけをサブエージェントに委譲するときの指示の型も持つ。
---
