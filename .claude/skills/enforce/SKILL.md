...
**検査を足す issue は、畳んで空いた枠にだけ入れる。** 検査を1本減らして閉じた issue を1つ引き当て、どれを引いたかを「現状」に書く。引ける枠が無ければ、足す issue は出さない。上限が無いと、毎週かかるのは足す力だけになる。

**同じ枠は2度引けない。** 引かれているかは、その issue を参照している issue で分かる（→ 5）。
