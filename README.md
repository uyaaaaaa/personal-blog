...
配信物に影響しないファイルだけを触った push では、プレビューも本番もビルドされません。Cloudflare Pages の **Build watch paths**（ダッシュボードの Settings > Build）で、次を除外しています。
...
除外は push 単位ではなくパス単位です。除外パスを先に落とし、残りが1つでもあればビルドします。ドキュメントと実装を同じコミットで触った場合（このリポジトリでは推奨される形）は通常どおりビルドされるので、実装の変更がプレビューから漏れることはありません。

スキップされた push には `Cloudflare Pages` チェック自体が付きません（失敗ではなく不在です）。除外パスだけの変更でもビルドさせたいときは、配信物に影響するファイルを一緒に含めるか、ダッシュボードから対象コミットを再デプロイします。

逆に、除外パスだけの変更でもビルドが走ることがあります。ファイル変更が 0 件の push、3000 ファイル以上の push、20 コミット以上をまとめた push は、Cloudflare がパス判定を飛ばして必ずビルドします。
