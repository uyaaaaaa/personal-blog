...
配信物に影響しないファイルだけを触った push では、プレビューも本番もビルドされません。Cloudflare Pages の Build watch paths（ダッシュボードの Settings > Build）で、`docs/*` `.claude/*` `.github/*` `README.md` `CLAUDE.md` を除外しています。実装と同じコミットでドキュメントを触った場合は、除外に当たらないパスが残るので通常どおりビルドされます。

スキップされた push には `Cloudflare Pages` チェック自体が付きません。失敗ではなく不在なので、`main` のブランチ保護でこのチェックを必須にはできません。
