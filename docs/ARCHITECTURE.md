...
commit のたびに回すのは lint だけにし、テストと型検査と build は PR で受ける。lint は見ている対象で分け、記事（`content/`）しか変えていないコミットと PR では、コードのための検査と workflow を回さない（→ [ADR 21](./adr/21-checks-split-by-changed-paths.md)）。lint で落とせるようになったルールは `.claude/rules/` から消す。
