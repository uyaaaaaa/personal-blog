...
| `/category/[category]` | ページヘッダー + 絞り込んだ記事グリッド（9件ごとのページャ）。未知のカテゴリと0件は404。全カテゴリの一覧ページは持たない（**索引はヘッダーの `Explore` が担う**）。 |
...
- **棚に出すカードが0枚になったら棚ごと出さない。** 記事が0件のカテゴリだけでなく、記事1本のカテゴリでその1本がHeroになった場合も含む。見出しと `View All` だけが残った棚は、`View All` が何に対する "All" なのか伝わらず、「記事が無い」という誤ったシグナルになるため（→ 設計原則「無駄な装飾を徹底的に排除する」）。
- **棚は「見せ場」であり、カテゴリの索引ではない。** 棚が消えてもカテゴリが辿れなくならないよう、索引はヘッダーの `Explore` が持つ（→ [COMPONENT_SPEC.md](./COMPONENT_SPEC.md) の `HeaderNavigation`）。TOPの見せ方と、カテゴリの到達可能性・静的生成の対象を互いに独立させる。
- 棚が消えるのはその記事がHeroにいるときなので、**Heroにもカテゴリ名のチップを出す**。TOPだけを見た読者にもカテゴリという軸の存在が見える状態を保つ。チップはリンクにしない（Heroはカード全体が1つのクリックターゲットのため）。
