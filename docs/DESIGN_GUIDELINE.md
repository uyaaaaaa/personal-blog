...
1. **色ではなくボーダーと余白で区切る。** 面を塗って区切らない。常時影のついた要素は作らない。例外は Callout と `diff` コードブロックの行背景だけで、いずれも意図と根拠を配色の節に書いている。
...
| **diff 追加行の背景** | `#E6FFEC` | `rgba(46, 160, 67, 0.15)` | — | `--color-diff-add-bg` | `diff` コードブロックの追加行の行背景（GitHub の diff 表示に合わせる）。 |
| **diff 削除行の背景** | `#FFEBE9` | `rgba(248, 81, 73, 0.15)` | — | `--color-diff-remove-bg` | `diff` コードブロックの削除行の行背景（同上）。 |

コードブロック内のトークン色も Shiki のテーマ（`github-light` / `github-dark`）由来でパレットの外側です。文字色は上書きせずテーマのまま使います（上書きすると構文色の情報源が2つに割れるため）。

`diff` の追加行・削除行だけは例外で、**行全体の背景を塗ります**（→ 下表）。設計原則1「面を塗って区切らない」に対する意図的な例外です。
`+` / `-` の文字色だけでは追加と削除の塊が視認しづらく、差分は「どの行が変わったか」を面で追う情報だと判断しました。行背景の色は GitHub の diff 表示に合わせています。

| | 行背景（ライト） | 行背景（ダーク） | 文字色に対するコントラスト比 |
| :--- | :--- | :--- | :--- |
| 追加 | `#E6FFEC` | `rgba(46, 160, 67, 0.15)` | ライト 4.4:1 / ダーク 9.1:1 |
| 削除 | `#FFEBE9` | `rgba(248, 81, 73, 0.15)` | ライト 5.9:1 / ダーク 7.9:1 |

**ライトの追加行だけが 4.5:1 をわずかに下回ります**（4.38:1）。行頭の `+` / `-` と行背景でも区別が付き色のみに依存しないため、許容しています。
