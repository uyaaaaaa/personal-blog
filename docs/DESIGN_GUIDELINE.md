...
1. **色ではなくボーダーと余白で区切る。** 面を塗って区切らない。常時影のついた要素は作らない。例外は Callout の背景と `diff` コードブロックの行背景だけで、いずれも意図と根拠を配色の節に書いている。
...
テーマ自身も diff のトークンに背景色を持っており（文字幅だけ塗られて見た目が崩れるため `ProsePre` で打ち消している）、行背景はそれを行幅に広げたものと位置づけています。
