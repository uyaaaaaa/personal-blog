...
| **先頭に戻る** | 記事詳細で `1画面分` と `ページ全体の1/3` の大きい方を超えて読み進めたら、右下に上向きシェブロンのボタンを浮かべる。濃さはスクロール量に連動させ、少しずつ現れて少しずつ消える。押すとページ先頭へスムーズスクロールし、URLのハッシュを取り除く（→ [COMPONENT_SPEC.md](./COMPONENT_SPEC.md) の `ScrollToTopButton`）。 | 実装済 |
