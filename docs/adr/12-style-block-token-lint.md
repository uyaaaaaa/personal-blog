...
- **対価**
  ...
  - **`<style>` の中に逃げ道が無い**: `eslint-disable` のコメントは `<style>` の中では効かない。抑制するなら `<script>` に置いてその下に効かせるしかなく（→ [ADR 20](./20-no-template-lint-escape.md)）、値の検査で例外を作る手段は `theme/tokens.ts` に名前を足すことだけになる（クラス側の任意値と同じ）。
  - **`em` と相対単位は見ない**: 語彙を持たないため対象外。`letter-spacing` を `em` で書けば長さの検査は通る。
- **戻す条件**: 「スタイルを `.css` に持つようになったら Stylelint に移す」としていたが、[ADR 17](./17-one-style-judgment-two-runners.md) で覆した。判定の置き場と戻す条件はそちらが持つ。
