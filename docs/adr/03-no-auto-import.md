...
- **検討した案**
  - **有効のまま**: 依存が `import` 文に現れず、循環検出や依存方向の lint が無言で無効になるため却下。
  - **`imports.autoImport: false`**: Nuxt / Vue の組み込み API まで止まり、`useColorMode` が解決できず 500 になるため却下（Nuxt 4.2 / `@nuxt/content` 3.9 で確認）。
  - **`scan: false`（採用）**: `app/composables/` `app/utils/` `shared/` の走査だけを止め、プリセット由来の `ref` `useAsyncData` `queryCollection` は解決されたまま。
