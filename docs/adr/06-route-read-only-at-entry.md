...
- **検討した案**
  ...
  - **route を読む composable の名前を lint の一覧に持つ**: `usePagination` のように route を読む composable を `components/` から呼べなくできるが、一覧は composable が増えるたびに手で足す必要があり、足し忘れが穴になる。
...
- **対価**
  - **入口が2階層になる**: 「route を読めるのは `pages/` だけ」とは言えず、`layouts/` を含めた入口の集合を覚える必要がある。
  - **props の中継が生える**: ヘッダーは受け取った現在地を自分でも使い、`HeaderNavigation` にも渡す。[ADR 05](./05-fetch-follows-route-dependency.md) が取得について退けた「中継だけの層」とは違い、中継する側も同じ値で閉じるが、階層が深くなれば同じ形の受け渡しは増える。
