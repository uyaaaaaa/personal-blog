...
SNS などにシェアされたときのカード画像。`public/ogp.png`（1200×630）を全ページ共通で出す。
記事ごとの差し替えはしない。

上部の帯 + 下線の中にヘッダーと同じロゴロックアップを置き、左端のアクセントバーと
`for Experts` の 1 行だけに `#8B5CF6` を使って、それ以外は無彩色に寄せている。
配色は `theme/tokens.ts` を追わず、下表の値で固定している。
