...
これに加えて `txt` はプレーンテキストとして常に使える（Shiki の特殊扱いで文法を必要としないため、`langs` への記載は不要）。
...
確認するならビルド後の `.data/content/contents.sqlite` を引き、`pre` の子 span に `class` が付いているかを見る。

Shiki のハイライトは**ビルド時に完結**し、文法もテーマもクライアントバンドルには載らない（トークンは `.data/content/contents.sqlite` に色クラス付きで格納され、色の実体は CSS に出る）。
そのため `langs` に言語を足しても閲覧者が受け取るバイト数は増えない。

**言語名の表記は1つに統一する。** `ts` / `yaml` のように `langs` に書いた綴りを使い、`typescript` / `yml` のようなエイリアスは使わない。
