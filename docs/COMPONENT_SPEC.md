...
| `useScrollSubscription(handler, enabled)` | `app/composables/useScrollSubscription.ts` | `handler` を `scroll` と `resize` に購読する。`enabled` が真になった時点で1度呼んでから購読し、偽になったら解除する。`display: none` で見えていないコンポーネントがスクロールのたびに計算するのを避けるためのもの（記事が長いほど見出し数に比例して効く） |
