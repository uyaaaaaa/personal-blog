...
| 経路 | 挙動 |
| :--- | :--- |
| ホバー | トリガーとパネルを含む `.explore` の `pointerenter` で開き、`pointerleave` から **150ms** 後に閉じる。パネルはトリガーの子孫なので、絶対配置でもこの判定に含まれる。**`pointerType` が `mouse` のときだけ**反応する。タッチはタップのたびに互換マウスイベントを発火させるため、これを入れないと1回目のタップで開いた直後にクリックのトグルが閉じてしまう。 |
| クリック / タップ | トリガーでトグル。`.explore` の外側のクリックで閉じる（ホバーできない環境向け）。タッチ端末ではこの経路だけで開閉する。 |
...
| `View All` | 見出しの右端。アクセント色・太字。ホバーで矢印が `translate-x-2`。**棚に出ていない記事が無いときは出さない。** 判定の基準はその幅で実際に見えている件数で、`1024px〜` は `total <= min(articles.length, 4)`、`〜1023px` は `total <= articles.length` のとき出さない。切り替えはJSのメディアクエリではなく `hidden lg:flex` で行う（カードを落とすのが `lg:hidden` なので、判定もCSS側に揃える） |
