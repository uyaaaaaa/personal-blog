...
4. `Read Article` ボタン（サブサーフェス色背景 / 1pxボーダー / 角丸 `0.375rem` / 等幅 / 太字 / サブテキスト色 / 右矢印アイコン）
...
- タイトルがアクセント色に変化
- `Read Article` ボタンの背景がミュートサーフェス色、文字がメインテキスト色、枠線がサブテキスト色に変化し、矢印が右に `translate-x-0.5` 移動（`200ms`）
