<template>
	<div
		...
		:style="{
			'--callout-rgb-light': colors.light,
			'--callout-rgb-dark': colors.dark,
			'--callout-surface-alpha': CALLOUT_SURFACE_ALPHA,
		}"
	>
		<component
		...
		>
			<Icon class="shrink-0">
				<g v-html="CALLOUT_ICONS[normalizedType]" />
			</Icon>
...
<script setup lang="ts">
	...
	import { CALLOUT_COLORS, CALLOUT_SURFACE_ALPHA, type CalloutType } from '~/utils/callout'
	...
	const normalizedType = computed<CalloutType>(() => {
		...
		if (type in CALLOUT_COLORS) return type as CalloutType
		// 未知のタイプはObsidianと同様にnote扱い
		return TYPE_ALIASES[type] || 'note'
	})

	const colors = computed(() => CALLOUT_COLORS[normalizedType.value])
...
<style scoped>
	.callout {
		background-color: rgba(var(--callout-rgb), var(--callout-surface-alpha));
	}
