...
const PROBE = [
	'const OVERLAYS = {',
	"\tsearch: { overlay: '.search-overlay', trap: '.search-dialog' },",
	'}',
	"const active = LINK + '.is-active'",
].join('\n')
...
describe('decide', () => {
	...
	it('OVERLAYS の外にある綴りは数えない', () => {
		const files = { 'app/components/ui/Pagination.vue': '<a class="is-active">' }
		expect(
			denied(create(), {
				touched: () => Object.keys(files),
				text: (path) => files[path] ?? ask.text(path),
			}),
		).toBeNull()
	})
