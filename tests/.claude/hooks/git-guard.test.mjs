...
describe('decide', () => {
	...
	it('pr-guard を通らない PR の作成を止める', () => {
		...
		expect(decide(bash("env -S 'gh pr' create"), ask)).toMatch('create_pull_request')
		expect(decide(bash("env --split-string='gh pr create'"), ask)).toMatch(
			'create_pull_request',
		)
		expect(decide(bash('FOO=a\\ b gh pr create'), ask)).toMatch('create_pull_request')
		...
		expect(decide(bash('echo ok # gh pr create'), ask)).toBeNull()
		expect(decide(bash("bash -c 'gh pr create'"), ask)).toMatch('create_pull_request')
		expect(decide(bash('gh pr revert 1'), ask)).toMatch('create_pull_request')
		expect(decide(bash('gh pr update-branch 1'), ask)).toMatch('マージ')
		expect(decide(bash('echo "$(gh pr create)"'), ask)).toMatch('create_pull_request')
